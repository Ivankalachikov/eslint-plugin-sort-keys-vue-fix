 ...
 */
 ...
 module.exports = {
   ...
   isVueFile(path, context) {
    const vueFilesPathEndings = context.options[1].vueFilesPathEndings;
    return [...vueFilesPathEndings, '.vue', '.jsx'].some((vueFilesPathEnding) => path.endsWith(vueFilesPathEnding))
    //  return path.endsWith('.vue') || path.endsWith('.jsx')
    //  return path.endsWith('.vue') || path.endsWith('.jsx')|| path.endsWith('.mixin.js')|| path.endsWith('script.js') 
   },
   ...
   isVueComponentFile(node, path, context) {
     return this.isVueFile(path, context) &&
   ...
   executeOnVueComponent(context, cb) {
     ...
     return {
       ...
       'ExportDefaultDeclaration:exit'(node) {
         // export default {} in .vue || .jsx
         if (!_this.isVueComponentFile(node, filePath, context) || isDuplicateNode(node.declaration)) return
         cb(node.declaration)
       },
